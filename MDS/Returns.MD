...
Using a Markov-Chain Sampling Model I predicted a total return percentage per faulty batch (with a ±2% uncertainty). I was even able to show, that regardless of starting severity, the returns would generally converge to this value. Using my model, I did an impact analysis detailing the maximum cost impact to the company while the faulty batches were still in circulation.
